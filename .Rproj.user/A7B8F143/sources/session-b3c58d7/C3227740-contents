  ...
  observe({
    if (!is.null(input$folder)) {
      ...
      output$folder_path <- renderPrint({
        ...
        output$qc.pf <- renderPlot({plot1qc + plot2qc})
        ...
        output$hvg.plot <- renderPlot({plot1h + plot2h})
        ...
        output$pca.plot <- renderPlot({dimplot})
